...
//#define EXTRAPOLATE_FROM_EDGE
  ...
  #if (LevelingBilinear::grid_points.x) < (LevelingBilinear::grid_points.y)
    #define HALF_IN_X
  #elif (LevelingBilinear::grid_points.y) < (LevelingBilinear::grid_points.x)
    #define HALF_IN_Y
  #endif
#endif
...
void LevelingBilinear::print_leveling_grid(const bed_mesh_t* _z_values/*=nullptr*/) {
  ...
  #define ABL_TEMP_POINTS_X (grid_points.x + 2)
  #define ABL_TEMP_POINTS_Y (grid_points.y + 2)

  #define LINEAR_EXTRAPOLATION(E, I) ((E) * 2 - (I))
  float LevelingBilinear::virt_coord(const uint8_t x, const uint8_t y) {
    uint8_t ep = 0, ip = 1;
    if (x > (grid_points.x) + 1 || y > (grid_points.y) + 1) {
  ...
  void LevelingBilinear::subdivide_mesh() {
    ...
    for (uint8_t y = 0; y < grid_points.y; ++y)
      for (uint8_t x = 0; x < grid_points.x; ++x)
